...
function emptyGame(){return {status:"idle",maxPlayers:4,players:{},playerOrder:[],pieces:createPieces([]),turnOrder:[],currentTurnIndex:0,ranking:[],forfeited:[],history:[],winner:null,activeUid:null,lastMoveBy:null};}
...
function createInitialRoom(code,maxPlayers,name){const uid=currentUser.uid,p={uid,name,color:"yellow",seat:0,joinedAt:Date.now()};return{code,status:"waiting",hostUid:uid,maxPlayers,createdAt:Date.now(),updatedAt:Date.now(),players:{[uid]:p},playerOrder:[uid],state:{...emptyGame(),status:"waiting",maxPlayers,hostUid:uid,players:{[uid]:p},playerOrder:[uid],pieces:createPieces([p]),history:[`Salon ${code} créé. En attente des joueurs.`]}};}
async function createRoom(){if(!await initFirebase())return;leaveRoom(false);const code=generateCode(),maxPlayers=Number(els.maxPlayers.value||4),name=(els.playerName.value||"Joueur").trim()||"Joueur",ref=roomRef(code),room=createInitialRoom(code,maxPlayers,name);try{await ref.set(room);roomCode=code;currentRef=ref;showCode(code);els.joinCode.value=code;saveOnlineSession();setOnlineStatus(`Salon ${code} créé. Partage le code.`);listenRoom(code);}catch(e){console.error(e);setOnlineStatus("Création impossible. Vérifie les règles Firebase V37.");}}
...
async function startRoom(){if(!currentRef||!isHost())return;const players=playerArray();if(players.length<2){setOnlineStatus("Il faut au moins 2 joueurs.");return;}const turnOrder=players.map(p=>p.uid),started={...gameState,status:"playing",turnOrder,currentTurnIndex:0,activeUid:turnOrder[0],pieces:createPieces(players),history:["Partie démarrée.",...(gameState.history||[])],lastMoveBy:currentUser.uid};await currentRef.update({status:"playing",updatedAt:Date.now(),state:started});}
